...
async function cmdGrantSudo(flags) {
  ...
  if (resp?.needs_root && resp.command) {
    // If running from an interactive TTY and host is reachable via ssh, attempt to apply automatically
    if (process.stdin.isTTY) {
      console.log('\nAttempting to apply sudoers change remotely via ssh (you will be prompted for passwords as needed)...');
      // Build a small script to run as root on the offerer. We'll send the same contents but avoid nested quotes.
      const script = `# generated by ssh-helper\necho 'Applying sudoers fragment for ${payload.username || 'UNKNOWN'}'\ncat > ${resp.filename} <<'EOF'\n# added by ssh-helper on ${new Date().toISOString()}\n${resp.sudoLine}\nEOF\nchmod 440 ${resp.filename}\nvisudo -c -f ${resp.filename}\n`;
      try {
        const { spawn } = await import('node:child_process');
        const ssh = spawn('ssh', [host, 'sudo', 'bash', '-s'], { stdio: ['pipe', 'inherit', 'inherit'] });
        ssh.stdin.write(script);
        ssh.stdin.end();
        await new Promise((resolve, reject) => ssh.on('close', (code) => code === 0 ? resolve() : reject(new Error('ssh remote command failed with code ' + code))));
        console.log('Remote sudoers applied (or visudo returned OK).');
        return;
      } catch (e) {
        console.error('Automatic remote apply failed:', e.message || e);
        console.log('\nFallback: run this on the offerer as root to apply:');
